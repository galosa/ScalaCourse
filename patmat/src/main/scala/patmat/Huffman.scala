...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case Nil => trees
    case _ :: Nil => trees
    case first :: second :: ts => (makeCodeTree(first, second) :: ts).sortBy(weight(_))
  }
  ...
  /**
    ...
    */
  def until[T](test: List[T] => Boolean, func: List[T] => List[T])(trees: List[T]): List[T] = {
    if (test(trees)) trees
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decoder(partTree: CodeTree, bits: List[Bit]): List[Char] = {
      partTree match {
        case Leaf(char, _) => char :: decoder(tree, bits)
        case Fork(left, right, _, _) =>
          if (bits.isEmpty) List[Char]()
          else {
            bits.head match {
              case 0 => decoder(left, bits.tail)
              case 1 => decoder(right, bits.tail)
            }
          }
      }
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encoder(partTree: CodeTree, text: List[Char]): List[Bit] = {
      if (text.isEmpty) List[Bit]()
      else
        partTree match {
          case Leaf(_, _) => encoder(tree, text.tail)
          case Fork(left, right, _, _) =>
            if (chars(left).contains(text.head)) 0 :: encoder(left, text)
            else 1 :: encoder(right, text)
        }
    }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertHelp(partTree: CodeTree): CodeTable = {
      partTree match {
